...
function buildDoc() {
  ...
  children.push(
    kvTable([
      ...
      ["Rugs (collection)", "Server-rendered clearance-style page that mirrors the legacy isberian.com layout. Left sidebar holds a search input, a \"Your Selections\" panel with removable × chips and a Clear button, and four collapsible facet groups (By Styles, By Color, By Size, By Origin) each with a +/- toggle. Right column carries a \"Search results · X pieces\" header, a Sort by dropdown (Newest, Title A→Z, Title Z→A, Size small→large, Size large→small), a Per page dropdown (12 / 24 / 50), the rug grid, and « ‹ Page X of Y › » pagination. Every control writes to the URL — q, sort, perPage, page, and facet params — so any filtered view is shareable, bookmarkable, and visible to crawlers with JavaScript off. Each card carries the SKU eyebrow (\"No. 17600\") above the title."],
      ["Rug detail", "Two-column layout on desktop. The image column is sticky (anchored just below the navbar) so the photograph stays visible while the right column scrolls. The photograph itself is capped at min(75vh, 720px) so a single image cannot dominate the viewport. Right column reading order: SKU eyebrow (the legacy \"No. 17600\" form — internal IDs strip the rug- prefix), title, structured description block, inline 3D viewer with toolbar and AR (QR code beside it), suggested-setting render, four-scene lifestyle row, request-a-quote form, similar rugs, human exit."],
  ...
  children.push(
    ...featureBlock({
      ...
      eyebrow: "AI surface",
      name: "Concierge (Ask)",
      what: "Two doors. On the home page, an \"Ask anything\" pill sits anchored over the hero carousel; clicking it expands a concierge card right-anchored over the carousel imagery — the conversation begins without a route change or a modal takeover. On every other page, a floating concierge pill sits in the lower-right of the viewport and expands in place. The card is width-capped at min(360px, calc(100vw - 2rem)) so it can never overflow on small viewports. The arrow / send button on the collapsed pill is always active: clicking it with an empty input expands the card in place and focuses the input, so the affordance never reads as dead. /discover remains as the full-screen surface for longer questions. The concierge holds short or long conversations, searches the live catalog, answers care and material questions from the curated knowledge base, identifies rugs from photos (with hand-off), classifies service requests, and offers to book a showroom visit or create a lead.",
      why: "Most visitors leave silently. A conversational front door catches the ones who would have closed the tab and routes them to a real interaction with the showroom. Two clearly distinct doors — the hero pill on the home page and the floating pill elsewhere — are sufficient; a third entry in the secondary navigation (the legacy \"Ask\" nav link) was clutter and has been removed.",
      ...
      acceptance: [
        ...
        "The site header's secondary navigation does not include an \"Ask\" link — only the hero pill and the floating pill expose the concierge.",
