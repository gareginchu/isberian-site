...
function buildDoc() {
  ...
  children.push(
    kvTable([
      ["Home", "Hero carousel, action tiles, Rugs by style, Step-by-step, Repair and restoration, Showrooms, Press, Newsletter."],
      ["Rugs (collection)", "Filterable grid of every available rug. Facets: origin, size band, color family, technique, status."],
      ["Rug detail", "Photograph, 3D viewer with AR, QR scan-to-AR, structured description block, imagined-in-your-room lifestyle row, request-a-quote form, similar rugs."],
      ["Discover (AI surface)", "Conversational concierge — full-screen surface for longer questions. Pill input persistent on every other page."],
  ...
  children.push(
    ...featureBlock({
      ...
      what: "A server-rendered grid of every available rug. Each card shows the rug photograph, title, origin, size, and a status badge (available, on memo, sold). Visitors can filter by origin, color family, size band, technique, and status.",
      why: "The catalog is the spine of the site. Search engines and AI assistants need to discover and index real inventory. Designers and trade partners need to scan quickly. Retail buyers need a clean browse path.",
      ...
      acceptance: [
        ...
        "Each card emits Product JSON-LD with at least: name, brand (Oscar Isberian Rugs), image, identifier.",
  ...
  children.push(
    ...featureBlock({
      ...
      eyebrow: "AI surface",
      name: "Concierge (Ask)",
      what: "A persistent pill at the bottom of every page (\"Ask anything\") and a full-screen surface at /discover. The concierge holds short or long conversations, searches the live catalog, answers care and material questions from the curated knowledge base, identifies rugs from photos (with hand-off), classifies service requests, and offers to book a showroom visit or create a lead.",
  ...
  children.push(
    ...featureBlock({
      ...
      what: "Each rug page renders an inline 3D model — visitor can rotate, zoom, and reset — built from the rug's photograph and physical dimensions. A QR code beside it opens augmented reality directly on the visitor's phone (Scene Viewer on Android, Quick Look on iOS) and places the rug on their floor at true scale. No app to install.",
      why: "Photographs cannot communicate size. AR closes that gap. It is the most direct answer to \"will this fit my room?\" the site can provide.",
      acceptance: [
        "AR launches without requiring app installation on both major mobile platforms.",
        "The rug's real-world dimensions drive the AR scale — within 5% of physical measurement.",
  ...
  children.push(
    ...featureBlock({
      ...
      what: "A full-width row below the rug description showing the same rug imagined in four different interiors — a library, a modern living room, under a dining table, and at the foot of a bed. Each image is generated by an image-to-image model conditioned on the rug's photograph; pattern, palette, and proportions survive. The row is clearly labelled \"Imagined in your room\" with a caption clarifying these are renderings.",
      why: "The live site shows generic stock photos at the bottom of each rug page — the same images regardless of rug. The new site replaces this with rug-aware lifestyle imagery, helping the visitor picture the piece in real interiors without confusing it with the rug's actual photograph.",
      never: "Substitute for the rug's product photo; carry a price or a sale; imply that a particular room is included with the rug.",
      acceptance: [
        "Caption explicitly notes the row contains renderings — the photograph above is the definitive image.",
        "Every rug has at least three of the four scenes successfully rendered before the row appears.",
        "Pages without enough rendered scenes simply do not display the row — no broken layouts.",
      ],
    }),
  );
  ...
  children.push(
    ...featureBlock({
      ...
      what: "An admin surface where the editor reviews AI-drafted rug descriptions (lead, palette names, design features, distinguishing notes), confirms or corrects each field, and gates publish. Origin, age, and knot-count claims remain flagged as preliminary until verified.",
      why: "AI-drafted copy is the speed multiplier; editor review is the quality and trust multiplier. The schema only earns its strength when the gate exists in operations, not just in code.",
      acceptance: [
  ...
  children.push(
    kvTable([
      ...
      ["Catalog data", "Postgres with vector-search extension (pgvector) for image and text embeddings. Ingestion from the upstream inventory feed runs daily."],
      ["Content (editorial)", "Headless CMS (Sanity) for journal, knowledge base entries, and editor-approved rug descriptions."],
      ["AI", "Anthropic Claude (sonnet-class) via the official SDK for orchestration, vision, and care answers. Image generation for the lifestyle row via Replicate (Flux Kontext)."],
  ...
  children.push(
    ...
    h3("Content workflow"),
    body(
      "AI drafts a rug description from the photographs and inventory metadata. The editor reviews each field — lead, palette names, design features, distinguishing notes — and either approves, edits, or rejects. Verified facts (age, origin, knot count) require an explicit verified-flag toggle. Approved fields publish; rejected fields are returned to draft. The full action log is retained.",
    ),
    h3("Catalog ingestion"),
    body(
      "The upstream inventory feed is ingested on a daily schedule. New rugs land as drafts; sold rugs are flagged within minutes; price fields, if present in the feed, are dropped at the ingestion boundary and never written to the new catalog database.",
    ),
  ...
  children.push(
    ...
    bullet("Room visualizer with the visitor's own room (visitor uploads a photo, sees the rug placed in their actual room). The lifestyle row in Section 5.7 is a precursor; full room-visualizer is Phase 2."),
    ...
    bullet("Native mobile app — the AR experience uses platform-native viewers (Scene Viewer, Quick Look); a wrapper app is not in Phase 1."),
  );
